 ...
 * 3. 가비지콜렉터가 대표적인 데몬스레드임
 * 4. 주로 무한 반복 형태로 구현함
 * 5. setDaemon(true) 데몬스레드로 설정함 => 생성상태에서 설정해야함.
 * 
 * 
 * FTP : 파일 전송 전문. FTPD : FTP 서버.
 */
class DaemonThread extends Thread {
	public void run() {
		while(true) {
			 ...
			 *   #21 : 스레드번호
             *   Thread-0 : 스레드 이름. 스레드이름을 지정하지 않으면 자동으로 이름 부여
...
public class MainEx01 {
	public static void main(String[] args) throws InterruptedException {
		...
		t1.start(); t2.start(); //Thread 의 시작 => Runnable
		Thread.sleep(2000);
		System.out.println("main 스레드 종료"); //데몬스레드도 종료됨
	}
}
