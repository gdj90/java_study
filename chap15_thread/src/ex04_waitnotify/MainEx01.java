 ...
 * 계좌에 잔액이 0보다 크면 mathor wait 상태
 * 
 * mother : 1 ~ 3만원 사이의 금액을 계좌에 입금. son 깨움 0 ~ 3초 sleep
 * son    : 잔액을 0으로 만들고, mother 깨움. 0 ~ 1초 sleep
 * 
 * wait(),notify,notifyAll()
 *  1. 동기화 영역(동기화블럭,동기화메서드)에서만 실행가능
 *     => 동기화 영역이 아닌 경우 IllegalMonitorStateException 예외발생
 *  2. Object 멤버임 => 모든 객체들은 호출이 가능함 
...
class Account { //일반 클래스
	...
	synchronized void input() { //동기화메서드. Mother스레드호출
		while(money > 0) {
			try { wait();}
			catch(InterruptedException e) {}
		}
		money = ((int)(Math.random() * 3) + 1) * 10000;
		notifyAll(); //son 스레드 깨움
		System.out.println
...
class Mother extends Thread {
	...
	public void run() {
		for(int i=0;i<20;i++) {
			account.input();
			try {
				sleep((int)Math.random() * 3000); //0 ~ 2999 sleep
			}catch(InterruptedException e) {}
		}
	}
}
