...
/*
 * 추상화 
 *   - 클래스 생성의 과정. 추상화 과정의 결과가 클래스 임.
 *     클래스란 사용자 정의 자료형이다.
 *     
 * 클래스
 *   필드(변수,명사적특징, 속성)
 *      클래스변수(static) : 클래스 정보 로드시 메모리 할당. 모든 객체의 공유변수
 *      인스턴스변수 : 객체화시 메모리 할당. 객체별로 다른 메모리 할당
 *   멤서드(함수, 동사적 특징, 기능)    
 *      클래스메서드(static) : 클래스 정보 로드시 실행 준비 완료
 *      인스턴스메서드 : 객체화시 실행 준비 완료
 *      
 *   클래스멤버 : 클래스변수 + 클래스 메서드   
 *   인스턴스멤버 : 인스턴스변수 + 인스턴스 메서드
 *   => 클래스멤버에서 인스턴스멤버 직접 호출불가 => 객체화필요
 *   
 *   생성자 : 객체화시 호출되는 메서드. 인스턴스 멤버임. 
 *          생성자의 이름은 클래스명과 같다. 리턴타입이 없다.
 ...
 *           메서드의 다형성이라고도 함
 *           
 *   this : 자기참조변수. 인스턴스 메서드에 지역변수로 자동 선언됨. 자기 참조값을 저장
 *   this() : 생성자. 같은 클래스의 다른 생성자 호출시 사용. 첫줄에 사용해야 함
 *   
 *   초기화블럭 :
 *       static 초기화블럭, 인스턴스 초기화블럭
 *======================================================================
 * 상속 : extends 예약어 사용
 *       기존의 클래스를 이용하여 새로운 클래스를 생성할 수 있다
 *       재사용성
 * 캡슐화: 접근제어자 (private, (default), protected, public)
 * 다형성: 객체의 다형성.
 *       참조변수의 형변환임.  
 */
