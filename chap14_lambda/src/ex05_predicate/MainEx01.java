...
public class MainEx01 {
	public static void main(String[] args) {
		...
		System.out.println("영어 점수가 60이상인 학생의 영어 평균:" + 
 		   avg(list,s->s.getEng() >= 60));
		System.out.println("경영학과 학생의 영어 평균:" + 
		 		   avg(list,s->s.getMajor().equals("경영")));
		System.out.println("컴공과 학생의 영어 평균:" + 
		 		   avg(list,s->s.getMajor().equals("컴공")));
	}
	private static double avg(List<Student> list, Predicate<Student> f) {
		int sum=0,cnt=0;
		for(Student s : list) {
			if(f.test(s)) {//"임꺽정",80,95,"컴공""
				sum += s.getEng(); // 70 + 75 + 80 
				cnt++; //3
			}
		}
