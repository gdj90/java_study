 ...
 *               양수 : 현재 객체 뒤. t객체가 앞쪽
 *   - 클래스로 직접 구현하여,클래스의 기본 정렬 방식 설정시 사용되는 인터페이스
 *  
 ...
 *         결과 : 음수 : t1이 앞, t2가 뒤
 *               양수 : t2가 앞, t1이 뒤
 *   - 기본정렬방식이 이미 존재하거나, 기본 정렬방식을 설정하지 않은 경우에도 사용가능
 *   - 정렬 실행시 동적으로 정렬방식을 설정할때 사용
 *   
...
public class MainEx07_sort {
	public static void main(String[] args) {
		...
		System.out.print("데이터 추가 후:");
		System.out.println(list);
		System.out.print("기본 정렬 후:");
		Collections.sort(list);
		System.out.println(list);
		System.out.print("기본 정렬의 역순:");
		Collections.sort(list,Comparator.reverseOrder());
		Collections.sort(list,Collections.reverseOrder());
		System.out.println(list);
		System.out.print("나이 순 정렬:");
		Collections.sort(list,(p1,p2)-> p1.age - p2.age);
		System.out.println(list);
		System.out.print("나이 역순 정렬:");
