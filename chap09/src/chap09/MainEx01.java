 ...
 *   2. 객체화 가능. 
 *   3. 멤버를 소유함(클래스멤버,인스턴스멤버,상수,생성자)
 *   4. Object의 하위 클래스. 다른클래스 상속 가능. 
 *   5. 바이트코드 생성됨 (외부클래스명$내부클래스명.class)
 * - 외부클래스의 멤버임
 *   1. 외부클래스의 private멤버에 접근 가능
...
class Outer1 {
	...
	void method() { //실행되면 내부클래스가 객체화 부분 실행
		class LocalInner { //지역내부클래스 
			int iv = 600;  //내부클래스의 인스턴스 멤버
			static int cv = 700;  //내부클래스의 클래스 멤버
			final static int MAX=800; //내부클래스의 클래스 멤버
		}
